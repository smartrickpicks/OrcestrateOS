...
23. **Analyst Triage Multi-Queue Dashboard (v1.5.5)**: Replaced single-queue view with multi-queue dashboard showing four stacked queue sections: QA Issues, Extraction Issues, Logic Issues, and Patch Queue. Each section has its own header with item count and table with columns: Type, Record, Field, Status, Last Updated, Actions. Patch Queue section includes pipeline status cards (Pending, At Verifier, At Admin, Clarification, Rejected, Approved) above its table. QA/Extraction/Logic queues are placeholder sections for future data sources. `clearStaleReviewerData()` runs before `setMode()` to prevent stale queue counts. `PATCH_REQUEST_STORE.update()` method added for status transitions.
