...
21. **Folder-Based Data Source (v1.4.23)**: First-time users see folder selection modal with 5 mocked analyst folders. Data Source drawer shows "Connected Folder" card when mapped, with "Change Folder" and disabled "Sync Now (V2)" buttons. Upload/Connect blocks removed. Search input disabled with "Coming Soon" badge. Saved Datasets renamed to "Locally Cached Files" with empty state message. Demo/sample dataset auto-seeding removed; existing ds-sample entries purged from localStorage on load. Active data source bar shows folder connection status.
