...
- RFI Modal from Reader selection: `_evRfiModalOpen(payload)` opens a dedicated modal with selected text preview (gold-bordered quote block, max 300 chars), required question textarea, "Assign to" dropdown populated from `_dbMembers || getDemoUsers()`, and priority chip selector (Normal/Medium/High). `_evRfiModalSubmit()` POSTs to `POST /workspaces/{ws_id}/rfis` with `metadata.evidence_text`, `metadata.priority`, `metadata.assigned_to`, `metadata.source='ev_reader_selection'`. On success: toast, PatchPanel context registration, optimistic RFI panel insert + reconcile refresh, AuditTimeline `reader_rfi_created` event. CSS classes prefixed `.ev-rfi-`, HTML in `#ev-rfi-modal-overlay`, JS functions prefixed `_evRfi*`. Modal state in `_evRfiModalPayload` global.
- RFI Panel (Evidence Details): `_evLoadRfisForRecord(wsId, recordId, optimisticItem)` is dedicated helper for RFI panel loading. Backend `GET /workspaces/{ws_id}/rfis?record_id=` now filters by `target_record_id`. Optimistic insert renders created item immediately before API reconcile. Role-based filtering via `_evRfiRoleFilter(rfis, role, userId)`: admin/architect see all, analyst sees own-authored + custody-owned, verifier sees assigned + custody-awaiting. Role source: `_evGetEffectiveRole()` delegates to `getEffectiveRole()` (supports simulation mode). User ID source: `_evGetCurrentUserId()` from session/demoUser. Item rendering via `_evRenderRfiItem()` shows: custody status (color-coded badge), question preview (60 chars), author/assignee labels, latest response preview, relative timestamp. Debug flag: `_evRfiDebug` (default false) logs context keys, request URL, server/filtered counts.
- Encoding Correction Modal: `_evCorModalOpen(payload)` opens "Fix Encoding Issue" modal when "Create Correction" is clicked on suspect/unreadable text (`moji_class` from `_evGetSelectionPayload()`). Shows old value (readonly textarea) + new value (editable textarea). Live lane preview via `_evCorClassify()` mirrors backend `_classify_correction()` rules. Policy: minor encoding fix (length delta ≤2, no number/currency/date changes) → `auto_applied` status; risky edits (numbers, currency [$€£¥%], dates, large delta) → `pending_verifier` status. `_evCorModalSubmit()` POSTs to existing `POST /documents/{doc_id}/corrections` endpoint with `metadata.source='ev_reader_encoding_fix'`, `metadata.moji_class`. Status badge displayed after submit: green "Auto-applied" or amber "Pending verifier". Non-mojibake text correction still uses PatchPanel flow. CSS prefixed `.ev-cor-`, HTML in `#ev-cor-modal-overlay`, JS functions prefixed `_evCor*`. Modal state in `_evCorModalPayload` global. AuditTimeline event: `reader_encoding_correction_created`.
